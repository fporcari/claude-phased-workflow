...
# `claude` binary over six scenarios: model/cap selection, repair success,
# repair failure, idempotent repair marker, fable->opus fallback, progress guard.
...
fixture2() {  # 2 phases, both low/sonnet-or-default
cat > .claude/MEMORY.md <<'EOF'
# Context: orch-test
Parent: main
Mode: autonomous
...
assert "no repair launched" '! grep -q "/repair-phase" .claude/invocations.log'
...
finish_setup; run
assert "repair called on fable cap 300" 'grep -q -- "-p /repair-phase --model fable --permission-mode auto --max-budget-usd 300" .claude/invocations.log'
assert "repair succeeded message" 'grep -q "Repair succeeded" out.log'
assert "loop continued to phase 2" '[ "$(grep -c "/auto-phase" .claude/invocations.log)" = 2 ]'
...
assert "only 1 auto-phase + 1 repair" '[ "$(grep -c "CALL:" .claude/invocations.log)" = 2 ]'
...
assert "no repair call" '! grep -q "/repair-phase" .claude/invocations.log'
...
assert "opus repair cap 200" 'grep -q -- "-p /repair-phase --model opus --permission-mode auto --max-budget-usd 200" .claude/invocations.log'
