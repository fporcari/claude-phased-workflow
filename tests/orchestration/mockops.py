...
s = open(mem).read()
op = sys.argv[1]
# The [!] block a plan-defect claim writes — shared by the ops that raise the
# claim and the ops that resolve it (repair, apply).
CLAIM_BLOCK = ('- [!] **Phase 1**: phase one\n'
...
if op == 'complete':
    ...
    s = s.replace('- [ ] **Phase 1**: phase one',
                  '- [>] **Phase 1**: phase one\n  > WIP: In execution since '
                  '2026-07-18T00:00:00Z — step 1 done, step 2 pending. commit: abc1234', 1)
elif op == 'resume1':
    s = s.replace('- [>] **Phase 1**: phase one',
    ...
    s = s.replace('- [ ] **Phase 1**: phase one', CLAIM_BLOCK, 1)
