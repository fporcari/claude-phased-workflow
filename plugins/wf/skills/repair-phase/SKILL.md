...
Same rules as `/execute-phase-agent` Step 4: green signal = test suite + linter on the touched files; up to **two fix attempts** with the no-progress detector; then re-check every item of `Done:` literally.
