...
After confirmation, launch the run in the **background** and watch it while it runs — the run stays attached to this session. Tee the launcher's output to a log **outside the repo**; a file inside `.phased/` would dirty the tree at the next phase's start and land in that phase's commit, which is exactly why the launcher itself writes no file. Every control file of this run is named from ONE prefix, `$T` below — `next-phase.py --transport` computes it, and the launcher computes the same one, so a second checkout carrying the same slug can never consume this run's signals:

```bash
T=$(python3 "${CLAUDE_PLUGIN_ROOT}/scripts/next-phase.py" --transport)
install -d -m 700 "$(dirname "$T")"
bash "${CLAUDE_PLUGIN_ROOT}/scripts/run-workflow.sh" 2>&1 | tee "$T-run.log"
```
...
**Plan-defect consult.** `phase-needs-foreman` is the one event with a return leg: the launcher is HOLDING the repair, polling `$T-foreman-answer` (`$T` = `next-phase.py --transport`, the run's own prefix) — with no deadline: holding costs nothing, and the field's one true claim was "dissolved" by the repair that a 600-second window handed it to while the user slept (`RUN_WORKFLOW_CONSULT_TIMEOUT=<seconds>` is the explicit opt-in bound). While it holds, this session CHECKS the claim against the code the `> Issue:` names — read the files and lines it cites, confirm or refute the premise — so the human decides on a verified finding, not on the child's word. Relay the `plan-defect?` question (the phase's `> Issue:` claim plus your check's verdict) to the foreman; when its reply arrives — `plan-defect: repair`, `plan-defect: apply` or `plan-defect: stop` (`foreman.md` → *Plan-defect claims*) — write it into the answer file, as the bare verb (`apply`) or as the reply line itself (`plan-defect: apply`): the launcher reads both, and reports anything else while it keeps holding. The file lives outside the repo, so writing it breaks no invariant. On `apply`, the applying is THIS session's work, per the protocol's apply road: apply exactly the claim's before-text → after-text edit to both contract copies (byte-identical), re-run the phase's `Done:`, on green flip the phase `[x]` (`> Done:` re-stated, `> Applied:` note, `> Issue:` kept) and commit `wf: plan defect phase N — applied — <one line>`, then write `green` into `$T-apply-outcome` — the launcher resumes with the next phase, no repair and no relaunch. The window (`RUN_WORKFLOW_APPLY_TIMEOUT`, default 900s) is a hard deadline: red Done or no time left → reset what was touched, write `red`, and let the repair judge the claim. When this chat IS the foreman, put the question's options to the user here (AskUserQuestion — *Apply the declared edit*, recommended when your check confirmed the claim and the `> Issue:` carries the edit as before-text → after-text / *Authorize repair*, recommended when it did not / *Stop the run*) and write the file from the answer. No reply → the run keeps holding and the question waits for the human; a stop request (`$T-stop-request`, below) during the hold ends the run instead. Never write a guess: a check that confirmed the claim is evidence to put before the human, not an answer to write in their place.
...
- all `[x]` → `/quality-check`, then `/finalize-workflow` — what the user's check and the pre-commit review turn up there is fixed by the foreman — QA fix and final touch — never appended as a phase per finding (`/quality-check` → *QA fixes*, *The final touch*); any `[!]` → read its `> Issue:`/`> Attempted:` notes first
