...
2. **Scope safety.** Sub-sessions run `--permission-mode auto`, which blocks the categories listed in `${CLAUDE_PLUGIN_ROOT}/refs/common.md`. A phase needing one → surface it: *"Phase X richiede `<command>`, che auto mode blocca. Opzioni: (a) rifrasare la fase per fermarsi prima — lo fai tu dopo, (b) rimuovere la fase, (c) farla a mano fuori da `/run-all-phases`."* Never silently rewrite the phase to hide it.
