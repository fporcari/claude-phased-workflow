...
**The board.** Where the `visualize` MCP server is present, the state of the plan and the next step render as an inline grid — one row per phase with its state, its `Run:` hint and its files — while coverage, drift and oversizing stay prose, because a grid shows a position well and argues a finding badly. **The grid plus its controls is the board; a grid without them is the old text report set in a table**, so their positions are fixed: refresh in the header row next to the phase count, the launch command in the eligible phase's row. Refresh re-runs the skill rather than redrawing the widget (a printed message is immutable, so the recomputed report prints below — press it on returning from a phase chat that just committed). Opening the phase is a chip, not a button: one click opens a *session of its own*, born in the plan's own root so a plan living in a worktree does not start its phase in the wrong tree; one chip, for the eligible phase only, since phases run strictly in order, and none at all for a plan whose branch has no checkout yet. No server → the plain text report, with the launch command spelled out.
