...
- [x] **Phase 3**: Rename /run-all-phases → /run-workflow
  > Done: skill dir + launcher script renamed (env var too: RUN_WORKFLOW_NO_BUDGET); RUNNER_SRC, bench.sh, kb-sync MAPPING/EMBEDDED + KB_ONLY history notes, RETIRED_NAMES, README and loop-engineering swept. Historical mentions kept: README:493 + install.sh SUPPORT_PATHS (old 4.0.0 file), benchmark results README. Suites 109/109 bash+zsh.
  > Files: plugins/phased-workflow/skills/run-workflow/ (renamed), plugins/phased-workflow/scripts/run-workflow.sh (renamed), plugins/phased-workflow/skills/{auto-phase,repair-phase,import-workflow,write-workflow}/SKILL.md, plugins/phased-workflow/refs/common.md, plugins/phased-workflow/refs/write-workflow-autonomous.md, plugins/phased-workflow/install.sh, tools/kb-sync.py, tests/orchestration/run_tests.sh, tests/benchmark/bench.sh, README.md, docs/loop-engineering.md
...
- [x] **Phase 7**: Retire parallel:N and group:N (H)
  > Done: selector simplified (TAG_RE vast-only, strict order in blockers, group_unit/unit: removed, validator message "malformed or retired tag (valid: vast)"); write-workflow sizing rewritten (Standard/Split/vast, merge-instead-of-group guidance), template examples replaced; execute-phase group-unit handling removed; refs/common.md concurrency caveat + tag semantics removed; autonomous ref updated; launcher comment + fallback NOTE reworded, S20a follows. Pre-5.0 plans degrade to warnings (verified). Suites 116/116 bash+zsh.
  > Files: plugins/phased-workflow/scripts/next-phase.py, plugins/phased-workflow/skills/write-workflow/SKILL.md, plugins/phased-workflow/skills/execute-phase/SKILL.md, plugins/phased-workflow/skills/resume-workflow/SKILL.md, plugins/phased-workflow/refs/common.md, plugins/phased-workflow/refs/write-workflow-autonomous.md, plugins/phased-workflow/scripts/run-workflow.sh, tests/orchestration/run_tests.sh
...
- [x] **Phase 8**: Agent launcher + finalize-workflow-agent (K)
  > Done: agent-session.sh ships (resolve/attach plan root, claude -p "/<skill>" with shipped prompt, tee to log/<skill>.txt); finalize-workflow-agent skill created (read-only: Done: re-check, whole-diff cross-phase review, structured STATE/FINDINGS/VERIFY-NOTES report, base skill cited); finalize-workflow Step 4 delegates to it when the plan lives in a worktree/other checkout, and its frontmatter gains Bash(bash:*) + Agent (the panel inconsistency fixed). kb-sync MAPPING + EMBEDDED updated. Suite 116/116.
  > Files: plugins/phased-workflow/scripts/agent-session.sh, plugins/phased-workflow/skills/finalize-workflow-agent/SKILL.md, plugins/phased-workflow/skills/finalize-workflow/SKILL.md, tools/kb-sync.py
...
- [x] **Phase 9**: -agent convention — execute-phase-agent + shared core + guard (L)
  > Done: refs/phase-execution.md holds the shared core (selection, implementation discipline, outcome formats, phase commit, WIP escape); auto-phase renamed execute-phase-agent and rewritten thin (82 lines: unattended constraints + "Base skill: execute-phase" + core reference); execute-phase references the core and drops its duplicated Step 6 formats; launcher PHASE_PROMPT and pre-2.1.139 fallback renamed; bench.sh, S21 fixture path, kb-sync (MAPPING + KB_ONLY history), RETIRED_NAMES all updated; S23 guard added (base citation + existence + 100-line ceiling, mutations re-run the same function). Suites 120/120 bash+zsh.
  > Files: plugins/phased-workflow/refs/phase-execution.md, plugins/phased-workflow/skills/execute-phase-agent/ (renamed), plugins/phased-workflow/skills/execute-phase/SKILL.md, plugins/phased-workflow/skills/run-workflow/SKILL.md, plugins/phased-workflow/skills/repair-phase/SKILL.md, plugins/phased-workflow/refs/common.md, plugins/phased-workflow/refs/write-workflow-autonomous.md, plugins/phased-workflow/scripts/run-workflow.sh, plugins/phased-workflow/scripts/next-phase.py, plugins/phased-workflow/install.sh, tools/kb-sync.py, tests/orchestration/run_tests.sh, tests/orchestration/check_allowlists.py, tests/benchmark/bench.sh, README.md, docs/loop-engineering.md
