...
The foreman closes the quality check itself, and a job that is one session is told so before it becomes a plan. Field case, the 3-accesso-ambulatorio-id run on Demetra: a 3-phase plan for one bug — where does an `accesso` get its `ambulatorio_id` — landed in 7 files (+302/−7) and was verified by hand. Then `/quality-check` ran its high-effort, recall-biased review, found 10 things, and its Step 5 said *fixing is delegated, then re-run `/quality-check`*; the 6.32.0 QA fix did not apply because its boundary was *one sentence, no new callable*, so every finding went to `/resume-workflow` as its own phase: phases 4–9, 39 files (+968/−426), two `[!]` repairs, a second review that found 10 more, phases 10–13. The loop does not converge by construction — a recall review finds ten things on any diff, ten phases produce a diff, the next review finds ten more — and it broke only when the user asked *"can't you do these yourself?"*, and the foreman did the four phases in its own chat in minutes. Twenty-six hours, 26 `wf:` commits, 13 phases for a fix that fit one session. Three changes. **The final touch** (`/quality-check` → *The final touch*): the review's findings are fixed by the foreman — ONE table, finding → fix → files, applied on the user's ok, one `wf: final touch — <N> corrections` commit, the table recorded in `notes.md` under `## Final touch` for the lessons pass — followed by the only re-check allowed: Light at effort `low` over the touched files plus the `verify.md` items they serve, never Extended or Panel again on the branch; the stamp gains `final touch <N corrections | none>`. **The QA-fix boundary is decisions, not size**: a report is a QA fix when no decision is open — the user's sentence or the plan's `Decisions:` already says what right looks like — whatever it takes to get there, a helper, a foreign-key rule, a seed included; a phase only for a decision the user does not give in one sentence when asked, or a surface the plan never built, and then ONE phase for all such findings, which is now the only thing `/resume-workflow` accepts from a quality check. `foreman.md`'s exception is widened to name both, `/run-workflow`'s closing pointer and `/finalize-workflow`'s lessons pass read both sections. **One chat, or a workflow**: the plan's quality — recon, `Pattern:`, a re-runnable `Done:`, contract tests first — belongs to the plan, not to the phase boundaries, and one session handed the same plan as its prompt gets it too, in a warm context that knows the whole diff where separate sessions duplicate each other's helpers (round 2's findings 7, 8 and 10 were exactly that). A workflow pays only for one of three reasons: the work does not fit one context; an intermediate result changes what comes next, so a human gate is owed between phases; it must run unattended, with checkpoints and repair. `/issue` Step 2 sizes the job on those three and, when none holds, closes by handing over the prompt for one fresh chat (`fable` or `opus`, `high`) instead of `/write-workflow`; `/write-workflow` still does the recon and builds the plan, then its gate becomes *One chat / Workflow* with **One chat** recommended when none holds — on that answer it writes a **one-chat brief** to `~/.phased/prompts/<slug>.md` — not the plan copied, a self-contained prompt for a chat that has read nothing else: objective, the decisions never to reopen, file → pattern lines, constraints, method (contract tests first, lint and test commands, loop until green), the merged `Done:`, the deliverable and two stop rules (an unlisted decision → ask; Done not green after two attempts → report) — no branch, no `.phased/`; the recon, the decisions and the contract tests it carries were already paid for by the planning. Between the two sits `Channel: in-chat` (6.30.0), which the Demetra run — relayed and autonomous for a one-session fix — never considered. S44 extends to the final touch in every consumer, by mutation; S63 pins the three reasons in both skills, the brief, its stop rules and the gate. `/write-workflow`'s closure stays at 1500/1500 and `foreman.md` at its line count: every added line is paid by a pure re-wrap. `docs/design-notes.md` → *The final touch, and the loop it closes* carries the numbers.

## 6.34.0 — 2026-09-03

The worktree opens at planning, not by accident at launch. Field observation: `/write-workflow` created `wf/<slug>` with `git switch -c` in the checkout it was run from, so that checkout was left standing on the workflow branch; the launcher creates a worktree only for a plan whose branch has no checkout at all, which the user reached only by switching back to the parent by hand before launching — otherwise the run occupied the checkout they were working in, and any `git switch` there broke it. The worktree existed, but it was reached through the state of a branch rather than through a choice. Step 4 of `/write-workflow` now opens the `wf/` branch in `.claude/worktrees/<slug>`, cut from the parent, by default: the checkout it was run from never leaves the parent, `.phased/` and the plan commit land in the worktree, and the closing message names the workspace where the phase chats open — `/run-workflow` and `/resume-workflow` find it from the root as before, since the launcher's attach path already handled a plan in another checkout. The branch line flips it to "in this checkout"; on `Channel: in-chat` that is the default instead, because that conversation IS the workspace; the adopt path never offers it, a branch already checked out cannot be added as a worktree. Second half, the GenroPy environment: `activate_gnr_context` writes the worktree's env (own `.gnr/`, own ports) into its `.claude/settings.local.json`, which Claude Code reads at session START — so the activation has to happen before any chat or sub-session opens in the worktree, and the launcher, which created the worktree and spawned the sub-sessions in one breath, never ran it: a `gnr` there targeted the main repo's paths. `/write-workflow` runs the activation right after `git worktree add`, and `run-workflow.sh` and `agent-session.sh` run it after theirs, all three only when the script is on PATH and announcing the outcome. The activation itself stops appending `.gnr/` to the worktree's `.gitignore`, which a phase commit's `git add -A` would have carried into the feature, and writes the common `info/exclude` instead. S62 pins the live path with a mock activation (created, copied settings, run from inside the worktree, announced; silent without the script), the real script's exclude-not-gitignore against a fake `~/.gnr`, and by mutation the command in the skill and the call in both scripts. `/write-workflow`'s closure sits at 1499/1500: its new lines are paid by pure re-wraps of `common.md`'s short paragraphs.
